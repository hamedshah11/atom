...
# --- Streamlit secrets → environment (must happen before importing backend) ---
if "openai_api_key" in st.secrets:
    os.environ["OPENAI_API_KEY"] = st.secrets["openai_api_key"]
if "model_all" in st.secrets:
    os.environ["MODEL_ALL"] = st.secrets["model_all"]
# (Optional) set VERBOSITY in secrets as well if needed, or default is "medium".
...
if run:
    ...
    with col2:
        st.divider()
        # Generate PDF of the analysis (no visuals, per updated requirements)
